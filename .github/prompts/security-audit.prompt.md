...
Next, ask to user which issues they want to fix by either replying "all", or a comma-separated list of issue IDs. After the user responds, run a separate sub-agent (#runSubagent) to fix each issue that the user has specified. Each sub agent should report back with a simple `subAgentSuccess: true | false`.